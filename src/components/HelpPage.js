...
const HelpPage = () => {
  return (
    <div className="container mx-auto py-4 flex gap-8 px-4">
        ...
        <section className="flex bg-white rounded-lg shadow w-1/2 flex-col border-2 border-white h-min sticky top-8">
          <img src={operatorsImage} alt="Tech support operators" className="object-cover w-full h-1/2 rounded-tr-lg rounded-tl-lg" />
          <div className="flex w-full sm:flex-row flex-col">
              <a href="#/" className="py-4 text-center rounded-bl-lg custom-bg-green text-white font-medium w-full shadow border-r-2 border-t-2 border-white flex items-center justify-center gap-2"><FontAwesomeIcon icon={faPhoneOffice} className="custom-text-darkblue" />Start a Call</a>
              <a href="/services" className="py-4 text-center custom-bg-darkblue text-white font-medium w-full shadow rounded-br-lg border-t-2 flex items-center justify-center gap-2"><FontAwesomeIcon icon={faHeadset} className="custom-text-green" />Start a Chat</a>
            </div>
        </section>
    </div>
  );
